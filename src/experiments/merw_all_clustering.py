...
def merw_sim(graph_path: str, rounds: int, annotations_per_edge: int):
    error = 0
    path_true = 'experiment_data/{}'.format(graph_path)
    path_out = '{}_results/merw/{}'.format(path_true, '{}')
    file_suffix = '.graph'
    ...
    for i, (name, path) in enumerate(paths_to_true):
        for _round in range(rounds):
            ...
            merw_rounds = int(5 / annotations_per_edge)
            merw_spr = 2

            # Sampler/s
            sampling_step = Sampling(annotations_per_edge=annotations_per_edge).add_sampling_strategie(multiple_edges_randomwalk, {'rounds': merw_rounds, 'sample_per_node': merw_spr, 'start': annotated_graph.get_last_added_node})\
...
if __name__ == '__main__':
    merw_sim(sys.argv[1], int(sys.argv[2]), int(sys.argv[3]))
