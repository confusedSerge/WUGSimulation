...
def dwug_sim(graph_path: str, rounds: int, annotations_per_edge: int):
    ...
    for i, (name, path) in enumerate(paths_to_true):
        for _round in range(rounds):
            ...
            name_metric_rs = '{}-{}-{}-dwug_rs_sbm'.format(name.replace(file_suffix, ''), _round + 1, annotations_per_edge)
            name_metric_rs_judgement = '{}-{}-{}-dwug_rs_sbm_j'.format(name.replace(file_suffix, ''), _round + 1, annotations_per_edge)
