...
def dwug_sim(graph_path: str, rounds: int, annotations_per_edge: int):
    ...
    for i, (name, path) in enumerate(paths_to_true):
        for _round in range(rounds):
            ...
            name_metric_rs = '{}-{}-{}-dwug_rs_cc_split'.format(name.replace(file_suffix, ''), _round + 1, annotations_per_edge)
            name_metric_rs_judgement = '{}-{}-{}-dwug_rs_cc_split_j'.format(name.replace(file_suffix, ''), _round + 1, annotations_per_edge)
            ...
            simulation = Simulation(600, break_on_sc=True, tail_write=True, verbose=True)\
                ...
                .add_step(clustering_step_cc_split)\
