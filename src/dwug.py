...
from simulation.clustering.adv_clustering_strategy import time_degrading_clustering
from simulation.clustering.adv_clustering_strategy import time_degrading_clustering_reset
...
for graph in graphs:
    ...
    clustering_step = Clustering().add_adv_clustering_strategy(time_degrading_clustering, {'weights': 'edge_soft_weight'}, time_degrading_clustering_reset)
    ...
    listener = IntermediateSaveListener().add_listener(checkpoints, path_intermediate_graph, 'dwug_n{}_k{}'.format(graph.get_number_nodes(), graph.get_number_communities()), annotated_graph.get_num_added_edges)

    simulation = Simulation(1000).add_step(sampling_step).add_step(clustering_step).add_step(listener).add_step(stopping_step)
    ...
    with open('{}/dwug_n{}_k{}.graph'.format(path_out, graph.get_number_nodes(), graph.get_number_communities()), 'wb') as file:
