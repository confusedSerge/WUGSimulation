...
def time_degrading_clustering(graph: BaseGraph, params: dict) -> dict:
    ...
    clusters, stats = cluster_correlation_search(G=graph.get_nx_graph_copy(weights), s=s, max_attempts=int(
        max_attempts / 2**_current_run), max_iters=int(max_iters / 2**_current_run), initial=initial, split_flag=split_flag)
