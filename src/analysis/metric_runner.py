...
class MetricRunner():
    ...
    def run(self, graphs: list, reference_graphs: list) -> None:
        ...
        for step in self.steps:
            ...
            self.metric_result.add_metric_and_values(step[1].__name__ if step[3] is None else step[3], _graphs_metric)

    def _calc(self, step: tuple, graph: BaseGraph, ref_graph: BaseGraph) -> float:
        if len(step) != 4 or not callable(step[1]) or type(step[2]) != dict:
            return 0.0
        try:
            if step[0] == 'simple':
