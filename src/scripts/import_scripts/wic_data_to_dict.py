...
def gen_node_edge_dictionaries(input_file: str, score_file: str, tag_file: str):
    ...
    for k, obj in edge_info.items():
        # build new nodes
        if node_id_dict.get(obj['sentence1'], None) is None:
        ...
        if node_id_dict.get(obj['sentence2'], None) is None:
            new_node_id = len(node_id_dict)
        ...
        _edge_info = {'id': k, 'score1': list(map(float, score_info[k]['score']))[0], 'score2': list(
            map(float, score_info[k]['score']))[1], 'tag': tag_info[k]['tag']}
