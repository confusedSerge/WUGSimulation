...
path_out = 'data/graphs/kw39/sim/pr/{}'
file_suffix = '.graph'
rounds = 1
...
for i, (name, path) in enumerate(paths_to_true):
    for _round in range(rounds):
        ...
        # Sampler/s
        sampling_step = Sampling(annotations_per_edge=1).add_sampling_strategie(page_rank, {'sample_size': 10, 'start': annotated_graph.get_last_added_node, 'tp_coef': 0.1})\
        ...
        metric_sbm = MetricListener('{}-{}-pr_1_sbm'.format(name.replace(file_suffix, ''), _round), path_out.format(
            '{}-{}-pr_1_sbm'.format(name.replace(file_suffix, ''), _round)), checkpoints, annotated_graph.get_num_added_edges)\
        ...
        listener_sbm = IntermediateSaveListener()\
            .add_listener(checkpoints, path_out.format('{}-{}-pr_1_sbm'.format(name.replace(file_suffix, ''), _round)),
                          '{}-{}-pr_1_sbm'.format(name.replace(file_suffix, ''), _round), annotated_graph.get_num_added_edges)\
            .save_draw()\
            .add_preprocessing_step(clustering_step_sbm)
