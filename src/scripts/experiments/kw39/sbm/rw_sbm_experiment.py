...
path_out = 'data/graphs/kw39/sim/rw/{}'
file_suffix = '.graph'
rounds = 1
...
for i, (name, path) in enumerate(paths_to_true):
    for _round in range(rounds):
        ...
        metric_sbm = MetricListener('{}-{}-rw_1_sbm'.format(name.replace(file_suffix, ''), _round), path_out.format(
            '{}-{}-rw_1_sbm'.format(name.replace(file_suffix, ''), _round)), checkpoints, annotated_graph.get_num_added_edges)\
        ...
        listener_sbm = IntermediateSaveListener()\
            .add_listener(checkpoints, path_out.format('{}-{}-rw_1_sbm'.format(name.replace(file_suffix, ''), _round)),
                          '{}-{}-rw_1_sbm_j'.format(name.replace(file_suffix, ''), _round), annotated_graph.get_num_added_edges)\
            .save_draw()\
            .add_preprocessing_step(clustering_step_sbm)
