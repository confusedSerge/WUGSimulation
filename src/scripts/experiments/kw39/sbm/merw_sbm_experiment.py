...
path_out = 'data/graphs/kw39/sim/merw/{}'
file_suffix = '.graph'
rounds = 1
...
for i, (name, path) in enumerate(paths_to_true):
    for _round in range(rounds):
        ...
        # Sampler/s
        sampling_step = Sampling(annotations_per_edge=1).add_sampling_strategie(multiple_edges_randomwalk, {'rounds': 2, 'sample_per_node': 5, 'start': annotated_graph.get_last_added_node})\
        ...
        metric_sbm = MetricListener('{}-{}-merw_1_sbm'.format(name.replace(file_suffix, ''), _round), path_out.format(
            '{}-{}-merw_1_sbm'.format(name.replace(file_suffix, ''), _round)), checkpoints, annotated_graph.get_num_added_edges)\
        ...
        listener_sbm = IntermediateSaveListener()\
            .add_listener(checkpoints, path_out.format('{}-{}-merw_1_sbm'.format(name.replace(file_suffix, ''), _round)),
                          '{}-{}-merw_1_sbm'.format(name.replace(file_suffix, ''), _round), annotated_graph.get_num_added_edges)\
            .save_draw()\
            .add_preprocessing_step(clustering_step_sbm)
