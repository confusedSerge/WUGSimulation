...
for i, (name, path) in enumerate(paths_to_true):
    for _round in range(rounds):
        ...
        clustering = Clustering().add_clustering_strategy(new_correlation_clustering, {'weights': 'edge_soft_weight', 'max_attempts': 1000, 'max_iters': 5000, 'split_flag': False, 'cores': 8})

        metric = MetricListener('{}-{}-dwug_rand_1_cc_nosplit'.format(name.replace(file_suffix, ''), _round), path_out.format(
            '{}-{}-dwug_rand_1_cc_nosplit'.format(name.replace(file_suffix, ''), _round)), checkpoints, annotated_graph.get_num_added_edges)\
            .add_simple_metric('bootstrap_jsd', bootstraping_jsd, bootstraping_jsd_param)\
            .add_simple_metric('gambette_01', bootstraping_perturbation_ari, {'share': 0.1, 'clustering_func': new_correlation_clustering, 'clustering_params': {'weights': 'edge_soft_weight', 'max_attempts': 1000, 'max_iters': 5000, 'split_flag': False, 'cores': 8}})\
        ...
        listener = IntermediateSaveListener()\
            .add_listener(checkpoints, path_out.format('{}-{}-dwug_rand_1_cc_nosplit'.format(name.replace(file_suffix, ''), _round)),
                          '{}-{}-dwug_rand_1_cc_nosplit_j'.format(name.replace(file_suffix, ''), _round), annotated_graph.get_num_added_edges)\
            .save_draw()
