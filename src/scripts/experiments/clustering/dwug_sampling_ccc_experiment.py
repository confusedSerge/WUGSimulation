...
for i, (name, path) in enumerate(paths_to_true):
    for _round in range(rounds):
        ...
        clustering = Clustering().add_clustering_strategy(connected_components_clustering, {'weights': 'edge_soft_weight'})

        metric = MetricListener('{}-{}-dwug_ccc'.format(name.replace(file_suffix, ''), _round), path_out.format(
            '{}-{}-dwug_ccc'.format(name.replace(file_suffix, ''), _round)), checkpoints, annotated_graph.get_num_added_edges)\
        ...
        listener = IntermediateSaveListener()\
            .add_listener(checkpoints, path_out.format('{}-{}-dwug_ccc'.format(name.replace(file_suffix, ''), _round)),
                          '{}-{}-dwug_ccc_j'.format(name.replace(file_suffix, ''), _round), annotated_graph.get_num_added_edges)\
