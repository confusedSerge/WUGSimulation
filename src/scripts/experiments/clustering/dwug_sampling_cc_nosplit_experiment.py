...
for i, (name, path) in enumerate(paths_to_true):
    for _round in range(rounds):
        ...
        clustering = Clustering().add_clustering_strategy(new_correlation_clustering, {
            'weights': 'edge_soft_weight', 'max_attempts': 10, 'max_iters': 200, 'split_flag': False})

        metric = MetricListener('{}-{}-dwug_cc_nosplit'.format(name.replace(file_suffix, ''), _round), path_out.format(
            '{}-{}-dwug_cc_nosplit'.format(name.replace(file_suffix, ''), _round)), checkpoints, annotated_graph.get_num_added_edges)\
        ...
        listener = IntermediateSaveListener()\
            .add_listener(checkpoints, path_out.format('{}-{}-dwug_cc_nosplit'.format(name.replace(file_suffix, ''), _round)),
                          '{}-{}-dwug_cc_nosplit_j'.format(name.replace(file_suffix, ''), _round), annotated_graph.get_num_added_edges)\
