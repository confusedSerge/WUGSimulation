...
for i, (name, path) in enumerate(paths_to_true):
    for _round in range(rounds):
        ...
        # Listeners LM
        clustering_step_lm = Clustering().add_clustering_strategy(louvain_method_clustering, {'fmap': lambda x: (x - 1) / 3})
        ...
        metric_lm = MetricListener('{}-{}-modifiedrandomwalk_1_lm'.format(name.replace(file_suffix, ''), _round), path_out.format(
            ...
            .add_simple_metric('gambette_01', bootstraping_perturbation_ari, {'share': 0.1, 'clustering_func': louvain_method_clustering, 'clustering_params': {'fmap': lambda x: (x - 1) / 3}})\
