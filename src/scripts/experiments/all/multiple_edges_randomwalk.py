...
path_out = 'data/graphs/kw34/sim/merandomwalk/{}'
file_suffix = '.graph'
rounds = 10
...
for i, (name, path) in enumerate(paths_to_true):
    for _round in range(rounds):
        ...
        # Sampler/s
        sampling_step = Sampling().add_sampling_strategie(multiple_edges_randomwalk, {'rounds': 2, 'sample_per_node': 5, 'start': annotated_graph.get_last_added_node})\
        ...
        metric_cc_split = MetricListener('{}-{}-merandomwalk_cc_split'.format(name.replace(file_suffix, ''), _round), path_out.format(
            '{}-{}-merandomwalk_cc_split'.format(name.replace(file_suffix, ''), _round)), checkpoints, annotated_graph.get_num_added_edges)\
        ...
        listener_cc_split = IntermediateSaveListener()\
            .add_listener(checkpoints, path_out.format('{}-{}-merandomwalk_cc_split'.format(name.replace(file_suffix, ''), _round)),
                          '{}-{}-merandomwalk_cc_split_j'.format(name.replace(file_suffix, ''), _round), annotated_graph.get_num_added_edges)\
            .add_preprocessing_step(clustering_step_cc_split)
        ...
        metric_cc_nsplit = MetricListener('{}-{}-merandomwalk_cc_nosplit'.format(name.replace(file_suffix, ''), _round), path_out.format(
            '{}-{}-merandomwalk_cc_nosplit'.format(name.replace(file_suffix, ''), _round)), checkpoints, annotated_graph.get_num_added_edges)\
        ...
        listener_cc_nsplit = IntermediateSaveListener()\
            .add_listener(checkpoints, path_out.format('{}-{}-merandomwalk_cc_nosplit'.format(name.replace(file_suffix, ''), _round)),
                          '{}-{}-merandomwalk_cc_nosplit_j'.format(name.replace(file_suffix, ''), _round), annotated_graph.get_num_added_edges)\
            .add_preprocessing_step(clustering_step_cc_nsplit)

        # Listeners CC
        clustering_step_ccc = Clustering().add_clustering_strategy(connected_components_clustering, {'weights': 'edge_soft_weight'})

        metric_ccc = MetricListener('{}-{}-merandomwalk_ccc'.format(name.replace(file_suffix, ''), _round), path_out.format(
            '{}-{}-merandomwalk_ccc'.format(name.replace(file_suffix, ''), _round)), checkpoints, annotated_graph.get_num_added_edges)\
        ...
        listener_ccc = IntermediateSaveListener()\
            .add_listener(checkpoints, path_out.format('{}-{}-merandomwalk_ccc'.format(name.replace(file_suffix, ''), _round)),
                          '{}-{}-merandomwalk_ccc_j'.format(name.replace(file_suffix, ''), _round), annotated_graph.get_num_added_edges)\
            .add_preprocessing_step(clustering_step_ccc)

        # Listeners CW
        clustering_step_cw = Clustering().add_clustering_strategy(chinese_whisper_clustering, {'weights': 'edge_soft_weight'})

        metric_cw = MetricListener('{}-{}-merandomwalk_cw'.format(name.replace(file_suffix, ''), _round), path_out.format(
            '{}-{}-merandomwalk_cw'.format(name.replace(file_suffix, ''), _round)), checkpoints, annotated_graph.get_num_added_edges)\
        ...
        listener_cw = IntermediateSaveListener()\
            .add_listener(checkpoints, path_out.format('{}-{}-merandomwalk_cw'.format(name.replace(file_suffix, ''), _round)),
                          '{}-{}-merandomwalk_cw_j'.format(name.replace(file_suffix, ''), _round), annotated_graph.get_num_added_edges)\
            .add_preprocessing_step(clustering_step_cw)

        # Listeners LM
        clustering_step_lm = Clustering().add_clustering_strategy(louvain_method_clustering, {})

        metric_lm = MetricListener('{}-{}-merandomwalk_lm'.format(name.replace(file_suffix, ''), _round), path_out.format(
            '{}-{}-merandomwalk_lm'.format(name.replace(file_suffix, ''), _round)), checkpoints, annotated_graph.get_num_added_edges)\
        ...
        listener_lm = IntermediateSaveListener()\
            .add_listener(checkpoints, path_out.format('{}-{}-merandomwalk_lm'.format(name.replace(file_suffix, ''), _round)),
                          '{}-{}-merandomwalk_lm_j'.format(name.replace(file_suffix, ''), _round), annotated_graph.get_num_added_edges)\
            .add_preprocessing_step(clustering_step_lm)
