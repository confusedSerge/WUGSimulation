...
for i, (name, path) in enumerate(paths_to_true):
    for _round in range(rounds):
        ...
        clustering = Clustering().add_clustering_strategy(louvain_method_clustering, {})

        metric = MetricListener('{}-{}-dwug_lm'.format(name.replace(file_suffix, ''), _round), path_out.format(
            '{}-{}-dwug_lm'.format(name.replace(file_suffix, ''), _round)), checkpoints, annotated_graph.get_num_added_edges)\
        ...
        listener = IntermediateSaveListener()\
            .add_listener(checkpoints, path_out.format('{}-{}-dwug_lm'.format(name.replace(file_suffix, ''), _round)),
                          '{}-{}-dwug_lm_j'.format(name.replace(file_suffix, ''), _round), annotated_graph.get_num_added_edges)\
