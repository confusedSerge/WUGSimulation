...
out_path = 'data/graphs/sim_graphs/randomsampling/sim_ks_loghard'
...
for graph in graphs:
    ...
    listener = IntermediateSaveListener().add_listener(checkpoints, path_intermediate_graph, 'randomsampling_n{}_k{}'.format(graph.get_number_nodes(), graph.get_number_communities()), annotated_graph.get_num_added_edges)
    ...
    with open('{}/randomsampling_n{}_k{}.graph'.format(path_out, graph.get_number_nodes(), graph.get_number_communities()), 'wb') as file:
