...
def input_json_generator(id_prefix: str, lang: str, path_file: str, path_out: str):
    ...
    dict_objects = [_gen_dict_object('{}.{}'.format(id_prefix, i), wu1, wu2) for i, (wu1, wu2) in enumerate(wu_pairs)]
...
def _gen_dict_object(id_: str, sentence1: list, sentence2: list) -> dict:
    ...
    return {"id": id_, "lemma": sentence1[0], "pos": pos, "sentence1": sentence1[2], "sentence2": sentence2[2], "start1": str(sentence1[3]), "end1": str(sentence1[4]), "start2": str(sentence2[3]), "end2": str(sentence2[4])}

def _clean_lemma(lemma: str) -> str:
    return lemma.split('_')[0].strip()

def _lookup_pos_maingroup_german(pos: str) -> str:
    if pos.lower().startswith('n'): return 'NOUN'
    if pos.lower().startswith('v'): return 'VERB'
    if pos.lower().startswith('adj'): return 'ADJ'
    if pos.lower().startswith('adv'): return 'ADV'
    return ''

def _lookup_pos_maingroup_english(pos: str) -> str:
    if pos.lower().startswith('n'): return 'NOUN'
    if pos.lower().startswith('v'): return 'VERB'
    if pos.lower().startswith('j'): return 'ADJ'
    if pos.lower().startswith('r'): return 'ADV'
    return ''

def _lookup_pos_maingroup_swedish(pos: str) -> str:
    return ''

_lookup_table_pos = {'german': _lookup_pos_maingroup_german, 'english': _lookup_pos_maingroup_english, 'swedish': _lookup_pos_maingroup_swedish}
